...
def main() -> None:
    ...
    for i, prompt_len in enumerate([32, 64, 96, 48, 80, 40]):
        engine.submit(
            ...
            max_new_tokens=8,
            arrival_ms=i * 0.5,
            prompt_text=f"demo-request-{i}",
        )

    metrics = engine.run()
    print("request_id,prompt_len,prompt_text,new_tokens,ttft_ms,tpot_ms,latency_ms,blocks")
    for req in metrics.requests:
        ...
        print(
            f"{req.request_id},{req.prompt_len},{req.prompt_text or ''},{req.generated_tokens},"
